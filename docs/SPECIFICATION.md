...
**プロジェクト名**: 弁天クラブ 新人離職防止システム（新人管理システム→Benten管理システムを経て、現: ホシコン）  
**ドメイン**: bentenclub.com  
**作成日**: 2026-06-04  
**バージョン**: 0.1（凍結・更新停止）
...
### 6.3 ヘッダー共通UI（報告センター・お知らせベル・バージョン表示）

管理画面の左サイドバー・右上ヘッダーは全ページ共通（`src/html/layout.ts`）。

- **報告センター**: 左サイドバーに独立項目として配置（設定ページ配下ではない）。金色（`--color-accent`）で常時ハイライト表示し、他項目より目立たせる。表示・アクセス権限は5つの報告権限（`settings.lost-items` / `settings.accidents` / `settings.violations` / `settings.general-reports` / `settings.handover-memos`）のいずれか1つでもあればOK（`permissions.ts`のPATH_PERMISSIONSは`|`区切りでOR指定に対応）
- **お知らせベル**: 右上（モバイルはヘッダー内）のベルアイコン
  - 未読件数を赤バッジで表示。バッジ数はページ読み込み時に1回だけ取得（常時ポーリングはしない。長時間開きっぱなし運用でのメモリ増加対策）
  - クリックでドロップダウン一覧を表示。開いた時点で表示中の全件を自動既読化
  - 本文が長い場合は一覧内で2行に省略表示し、項目をタップ（クリック）すると全文表示モーダルが開く
  - 各項目の「×」ボタンで個人単位に一覧から削除できる（既読状態のみ削除可能。announcements本体・配信履歴・他の管理アカウントの表示には影響しない。`admin_announcement_reads.dismissed_at`で管理）
  - お知らせの配信元は「設定 → お知らせ配信」ページ。配信先チャネル（LINEのみ／Web管理画面のみ／両方）を選べる。Web宛は全管理アカウント共通固定（個別対象指定はできない）
- **バージョン表示**: ベルアイコンの右側に `v0.0.01` のようなバッジを常時表示。`src/config.ts`の`APP_VERSION`定数で管理し、まとまった変更をデプロイするたびに手動で書き換える（自動採番はしない）。バージョン番号自体の増減ルールは特に定めず、変更の目安として運用者が判断して更新する
