...
// 車番検索管理者一覧（閲覧・強制解除）
// =====================
app.get('/settings/vehicle-admins', async (c) => {
  type AdminRow = { id: number; name: string; line_uid: string; created_at: string };
  const res = await c.env.DB.prepare(
  ...
  const rows = admins.length === 0
    ? `<tr><td colspan="4" style="padding:24px;text-align:center;color:#9ca3af;">登録されていません</td></tr>`
    : admins.map(a => `
    <tr>
...
app.delete('/api/vehicle-admins/:id', async (c) => {
  const id = parseInt(c.req.param('id'));
