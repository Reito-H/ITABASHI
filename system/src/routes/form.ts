// フォームページ & フォームAPI（LIFFなし版）
// ワンタイムセッショントークンで LINE UID を特定する
...
// セッション検証ヘルパー
async function validateSession(db: D1Database, token: string): Promise<string | null> {
  const row = await db.prepare(
    `SELECT line_uid FROM form_sessions WHERE token = ? AND expires_at > datetime('now')`
  ).bind(token).first<{ line_uid: string }>();
  return row?.line_uid ?? null;
}
...
// ===================================================
// フォームページ（GET /form/lost-item?t=TOKEN）
// ===================================================
app.get('/form/lost-item', async (c) => {
  const token = c.req.query('t') ?? '';
  const uid = await validateSession(c.env.DB, token);
  if (!uid) return c.html(expiredPage());
  return c.html(lostItemFormPage(token));
});

app.get('/form/accident', async (c) => {
  const token = c.req.query('t') ?? '';
  const uid = await validateSession(c.env.DB, token);
  if (!uid) return c.html(expiredPage());
  return c.html(accidentFormPage(token));
});

// ===================================================
// API: 社員検索（GET /api/form/employees?q=...&t=TOKEN）
// ===================================================
app.get('/api/form/employees', async (c) => {
  const token = c.req.query('t') ?? '';
  const uid = await validateSession(c.env.DB, token);
  if (!uid) return c.json({ error: 'expired' }, 401);
...
// API: 忘れ物報告 送信
// ===================================================
app.post('/api/form/lost-item', async (c) => {
  ...
  const uid = await validateSession(c.env.DB, body.token);
  if (!uid) return c.json({ error: 'expired' }, 401);
  ...
  `).bind(
    ...
    body.item_description ?? null, body.pickup_location ?? null,
    body.dropoff_location ?? null, body.customer_name ?? null,
    body.customer_phone ?? null, body.return_method ?? null,
    body.notes ?? null, uid,
  ).run();

  // セッション削除（使い捨て）
  await c.env.DB.prepare('DELETE FROM form_sessions WHERE token = ?').bind(body.token).run();
...
// API: 事故報告 送信
// ===================================================
app.post('/api/form/accident', async (c) => {
  const body = await c.req.json<{
    token: string;
    received_at?: string;
    vehicle_no?: string;
  ...
  `).bind(
    ...
    body.employee_name ?? null, body.employee_emp_no ?? null,
    body.employee_division ?? null, body.employee_team ?? null,
    body.accident_type ?? null, body.location ?? null,
    body.car_status ?? null,
    body.substitute_requested ? 1 : 0,
    body.police_notified ? 1 : 0,
    body.passenger_delivered ? 1 : 0,
    body.additional_info ?? null, body.other_party_name ?? null, body.other_party_phone ?? null, summary, uid,
  ).run();
...
function buildLostItemSummary(body: Record<string, unknown>): string {
  const lines: string[] = [];
  lines.push(body.report_type === 'customer' ? '【客からの忘れ物問い合わせ】' : '【忘れ物報告】');
...
<body><div class="box">
  <div class="icon">⏰</div>
  <h2>リンクの期限が切れています</h2>
  <p>LINEに戻って、もう一度ボタンを押してください。<br>（リンクは10分間有効です）</p>
</div></body></html>`;
}

function lostItemFormPage(token: string): string {
...
<head>
  ...
  <style>
    *{box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
    ...
    .card-title{font-size:13px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.05em;margin-bottom:12px;}
    ...
    input[type=text],input[type=tel],input[type=time],textarea,select{width:100%;border:1px solid #d1d5db;border-radius:8px;padding:10px 12px;font-size:15px;font-family:inherit;background:#f9fafb;color:#111827;-webkit-appearance:none;appearance:none;outline:none;transition:border-color .15s,background .15s;}
    ...
    .type-btn{flex:1;text-align:center;padding:9px 8px;border-radius:8px;font-size:14px;font-weight:600;cursor:pointer;border:none;background:transparent;color:#6b7280;transition:all .15s;}
    .type-btn.active{background:white;color:#1e3a5f;box-shadow:0 1px 3px rgba(0,0,0,.12);}
    .emp-wrap{position:relative;}
    .emp-suggestions{position:absolute;top:100%;left:0;right:0;background:white;border:1px solid #d1d5db;border-radius:8px;z-index:10;box-shadow:0 4px 12px rgba(0,0,0,.12);max-height:200px;overflow-y:auto;margin-top:2px;display:none;}
    ...
    .customer-fields{display:none;}.customer-fields.visible{display:block;}
    .btn-submit{width:100%;background:#1e3a5f;color:white;border:none;border-radius:12px;padding:15px;font-size:16px;font-weight:700;cursor:pointer;margin-top:8px;transition:background .15s;}
    .btn-submit:active{background:#152d4a;}.btn-submit:disabled{background:#9ca3af;cursor:default;}
    ...
    .return-radio{display:flex;gap:12px;flex-wrap:wrap;}
...
<body>
  <div id="app">
    <div class="page" id="form-page">
      ...
      <div class="card">
        ...
        <div class="field">
          <div class="emp-wrap">
            <input type="text" id="emp-search" placeholder="氏名・社員番号で検索" autocomplete="off" oninput="empSearchDebounce()">
  ...
  (function(){
    var now = new Date();
    document.getElementById('received_at').value =
  ...
  function empSearchDebounce() {
    ...
    empTimer = setTimeout(doEmpSearch, 300);
  }
  ...
  function doEmpSearch() {
    ...
    fetch('/api/form/employees?q=' + encodeURIComponent(q) + '&t=' + encodeURIComponent(TOKEN))
      ...
      .then(function(data){
        ...
        sug.innerHTML = data.map(function(e){
          ...
          return '<div class="emp-item" onclick="selectEmp('+JSON.stringify(e).replace(/</g,'\\u003c')+')">'
            + '<div>'+e.name+'</div>'
  ...
  function selectEmp(e) {
    ...
    var div = e.division ? e.division+'課' : '';
    var team = e.team ? e.team+'班' : '';
    var sel = document.getElementById('emp-selected');
    sel.style.display = 'block';
    sel.textContent = '✓ ' + e.name + '（' + div + team + ' / ' + e.emp_no + '）';
  ...
  document.addEventListener('click', function(e){
    var sug = document.getElementById('emp-suggestions');
    var es = document.getElementById('emp-search');
    if (!es.contains(e.target) && !sug.contains(e.target)) sug.style.display='none';
  });
  ...
  function submitForm() {
    ...
    for (var i=0; i<radios.length; i++) { if (radios[i].checked) { returnMethod=radios[i].value; break; } }
    ...
    .then(function(data){
      if (data.ok) {
        ...
        alert(data.error === 'expired' ? 'リンクの期限が切れました。LINEで再度操作してください。' : '送信に失敗しました');
      }
    })
    .catch(function(){ btn.disabled=false; btn.textContent='送信する'; alert('通信エラーが発生しました'); });
  }
  </script>
</body>
</html>`;
...
function accidentFormPage(token: string): string {
  const t = JSON.stringify(token);
  return `<!DOCTYPE html>
<html lang="ja">
<head>
  ...
  <style>
    *{box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
    ...
    .field{margin-bottom:12px;}.field:last-child{margin-bottom:0;}
    label{display:block;font-size:13px;color:#374151;margin-bottom:5px;font-weight:500;}
    input[type=text],input[type=time],textarea{width:100%;border:1px solid #d1d5db;border-radius:8px;padding:10px 12px;font-size:15px;font-family:inherit;background:#f9fafb;color:#111827;-webkit-appearance:none;appearance:none;outline:none;}
    input:focus,textarea:focus{border-color:#dc2626;background:white;}
    textarea{resize:vertical;min-height:72px;}
    .emp-wrap{position:relative;}
    .emp-suggestions{position:absolute;top:100%;left:0;right:0;background:white;border:1px solid #d1d5db;border-radius:8px;z-index:10;box-shadow:0 4px 12px rgba(0,0,0,.12);max-height:200px;overflow-y:auto;margin-top:2px;display:none;}
    .emp-item{padding:10px 12px;font-size:14px;cursor:pointer;border-bottom:1px solid #f3f4f6;}
    .emp-item:last-child{border-bottom:none;}.emp-item:hover{background:#fef2f2;}
    .emp-meta{font-size:11px;color:#6b7280;margin-top:2px;}
    .emp-selected{font-size:13px;color:#059669;margin-top:4px;font-weight:600;}
    .toggle-group{display:flex;gap:10px;flex-wrap:wrap;}
    .toggle-btn{padding:8px 16px;border:2px solid #d1d5db;border-radius:8px;background:white;color:#374151;font-size:14px;font-weight:600;cursor:pointer;transition:all .15s;}
    ...
    .dep-section{display:none;}.dep-section.visible{display:block;}
...
<body>
  <div id="app">
    <div class="page" id="form-page">
      ...
      <div class="card">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
          <div class="field" style="margin-bottom:0;"><label>課</label>
            <input type="text" id="employee_division" readonly style="background:#f3f4f6;color:#6b7280;"></div>
          <div class="field" style="margin-bottom:0;"><label>班</label>
            <input type="text" id="employee_team" readonly style="background:#f3f4f6;color:#6b7280;"></div>
        </div>
      </div>
      ...
      <div class="card dep-section" id="dep-section">
        <div class="card-title">乗客・代車対応</div>
        <div class="check-row" id="passenger-check" style="display:none;">
  ...
  function setCarStatus(s) {
    currentCarStatus = s;
    ['kusha','jissha','geisha'].forEach(function(id){ document.getElementById('cs-'+id).className='toggle-btn'; });
    var m = {'空車':'kusha','実車':'jissha','迎車':'geisha'};
    if (m[s]) document.getElementById('cs-'+m[s]).className='toggle-btn active';
    var dep = document.getElementById('dep-section');
    var pc = document.getElementById('passenger-check');
    if (s==='実車'||s==='迎車') {
      dep.className='card dep-section visible';
      pc.style.display = s==='実車'?'flex':'none';
    } else { dep.className='card dep-section'; }
  }

  function empSearchDebounce() { clearTimeout(empTimer); empTimer=setTimeout(doEmpSearch,300); }
  ...
  function doEmpSearch() {
    ...
    fetch('/api/form/employees?q='+encodeURIComponent(q)+'&t='+encodeURIComponent(TOKEN))
      ...
      .then(function(data){
        if (!data||!data.length) { sug.style.display='none'; return; }
        sug.innerHTML = data.map(function(e){
          var div=e.division?e.division+'課':''; var team=e.team?e.team+'班':'';
          return '<div class="emp-item" onclick="selectEmp('+JSON.stringify(e).replace(/</g,'\\u003c')+')">'
            +'<div>'+e.name+'</div><div class="emp-meta">'+div+team+' / '+e.emp_no+'</div></div>';
        }).join(''); sug.style.display='block';
      }).catch(function(){ sug.style.display='none'; });
  }
  ...
  document.addEventListener('click',function(e){
    var sug=document.getElementById('emp-suggestions');
    if(!document.getElementById('emp-search').contains(e.target)&&!sug.contains(e.target)) sug.style.display='none';
  });

  function submitForm() {
    var btn=document.getElementById('btn-submit');
    btn.disabled=true; btn.textContent='送信中...';
    fetch('/api/form/accident',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({
        token:TOKEN,
    ...
    .then(function(data){
      if(data.ok){
        ...
        btn.disabled=false; btn.textContent='報告書を作成・送信';
        alert(data.error==='expired'?'リンクの期限が切れました。LINEで再度操作してください。':'送信に失敗しました');
      }
    })
    .catch(function(){ btn.disabled=false; btn.textContent='報告書を作成・送信'; alert('通信エラーが発生しました'); });
  }
  </script>
</body>
</html>`;
