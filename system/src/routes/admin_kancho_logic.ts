...
<div class="kl-body">
  ...
  <table class="kl-table">
    ...
    <tr><td>section</td><td>常にmain固定（2026-09-17に①②表・s1/s2区分を全廃止）</td></tr>
  ...
  <h3 id="kl-6">6. 名前D&amp;D並び替え</h3>
  <ul>
    <li><b>名前D&amp;D並び替え</b>: 編集モード中のみ有効。同じ役割グループ内のみ並び替え可。ドロップした瞬間に即時保存（一括保存ボタン待ちではない）</li>
    <li><b>印刷ボタンの位置（2026-08-08変更）</b>: 以前はツールバーに常設ボタンがあったが、⚙️歯車メニュー内の先頭項目（「印刷」）に格納された。閲覧権限のみでも利用可能</li>
  </ul>

  <h3 id="kl-7">7. 警告チェック機能</h3>
  <p>すべて警告表示のみで<b>保存はブロックしない</b>。対象は有効かつ内勤のメンバーのみ。</p>
  <ol>
