...
// .wrangler/tmp/bundle-wCgquv/strip-cf-connecting-ip-header.js
...
// .wrangler/tmp/bundle-wCgquv/middleware-loader.entry.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();

// .wrangler/tmp/bundle-wCgquv/middleware-insertion-facade.js
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
...
app.get("/settings/status", async (c) => {
  ...
  let empCount = 0;
  try {
    const res = await c.env.DB.prepare("SELECT COUNT(*) as cnt FROM employees").first();
    empCount = res?.cnt ?? 0;
    dbOk = true;
  } catch (e) {
      ...
      <div style="background:white;border-radius:10px;padding:16px 20px;box-shadow:0 1px 4px rgba(0,0,0,0.08);border:1px solid #e5e7eb;margin-bottom:12px;">
        ...
        <div style="display:flex;flex-direction:column;gap:8px;">
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;">
            ...
            ${dbOk ? `<span style="font-size:12px;font-weight:700;padding:3px 10px;border-radius:20px;background:#dcfce7;color:#16a34a;">\u6B63\u5E38\uFF08\u793E\u54E1 ${empCount}\u4EF6\uFF09</span>` : `<span style="font-size:12px;font-weight:700;padding:3px 10px;border-radius:20px;background:#fee2e2;color:#dc2626;" title="${escHtml(dbMsg)}">\u30A8\u30E9\u30FC</span>`}
          </div>
        </div>
      </div>
...
app2.get("/vehicles", async (c) => {
  ...
  const emptyMsg = searched ? `<tr><td colspan="6" style="padding:24px;text-align:center;color:#9ca3af;">\u300C${escHtml(q)}\u300D\u306E\u691C\u7D22\u7D50\u679C\u306F\u3042\u308A\u307E\u305B\u3093</td></tr>` : `<tr><td colspan="6" style="padding:24px;text-align:center;color:#9ca3af;">\u4E0A\u306E\u691C\u7D22\u30DC\u30C3\u30AF\u30B9\u306B\u756A\u53F7\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044</td></tr>`;
  const content = `
...
app2.post("/vehicles/xlsx-diff", async (c) => {
  const body = await c.req.json();
  ...
  for (const row of incoming) {
    ...
    if (!db)
      continue;
    const changes = [];
    for (const { key, label } of FIELDS2) {
      const oldVal = db[key];
    ...
    if (changes.length > 0)
      diffs.push({ radio_no: row.radio_no, id: db.id, changes });
  }
  return c.json({ rows: diffs });
});
app2.post("/vehicles/xlsx-import", async (c) => {
  const body = await c.req.json();
...
function calcAge(birthDate) {
  if (!birthDate)
    return null;
  const today = new Date(Date.now() + 9 * 60 * 60 * 1e3);
...
app3.get("/staff", async (c) => {
  ...
  if (filterDiv !== "all") {
    ...
    params.push(parseInt(filterDiv));
  }
  ...
  const [staffRows, upcomingRetirements] = await Promise.all([
    (params.length ? baseStmt.bind(...params) : baseStmt).all(),
    c.env.DB.prepare(`
  ...
  const enrollBtns = [["all", "\u5168\u54E1"], ["\u901A\u5E38", "\u901A\u5E38"], ["\u80B2\u4F11", "\u80B2\u4F11"], ["\u75C5\u6B20", "\u75C5\u6B20"], ["\u50B7\u75C5", "\u50B7\u75C5"], ["\u9577\u6B20", "\u9577\u6B20"]].map(
  ...
  const retireBtns = [["", "\u5168\u54E1"], ["candidate", "\u9000\u8077\u5019\u88DC"], ["soon", "30\u65E5\u4EE5\u5185"], ["has", "\u9000\u8077\u65E5\u3042\u308A"]].map(
    ([v, l]) => `<a href="${ADMIN_PATH}/staff?${makeFilter("retire", v, base)}" style="padding:4px 10px;border-radius:4px;font-size:12px;text-decoration:none;${filterRetirement === v ? "background:#92400e;color:white;" : v === "candidate" ? "background:#fee2e2;color:#dc2626;" : "background:#f3f4f6;color:#374151;"}">${l}</a>`
  ).join("");
  ...
  const rows = (staffRows.results ?? []).map((e) => {
    ...
    <tr data-id="${e.id}" data-bg="${rowBg}" data-hover="${rowHover}"
      ...
      <td style="${C}white-space:nowrap;" data-val="${retDateVal}">
        ${retDateVal ? `<span style="font-size:12px;color:${isRetiringSoon ? "#b45309" : "#6b7280"};">${escHtml(retDateVal)}</span>
             ${isRetiringSoon ? '<span style="background:#fef3c7;color:#92400e;padding:1px 5px;border-radius:3px;font-size:10px;font-weight:700;margin-left:3px;">\u4E88\u5B9A</span>' : ""}` : '<span style="color:#d1d5db;font-size:11px;">\u2014</span>'}
      </td>
...
app3.get("/staff/:id", async (c) => {
  ...
  if (filterDiv !== "all") {
    conditions.push("division = ?");
    navParams.push(parseInt(filterDiv));
  }
  const VALID_ENROLLMENT_FILTER = ["\u901A\u5E38", "\u80B2\u4F11", "\u75C5\u6B20", "\u50B7\u75C5", "\u9577\u6B20"];
  if (filterStatus !== "all" && VALID_ENROLLMENT_FILTER.includes(filterStatus)) {
    conditions.push("enrollment_status = ?");
    navParams.push(filterStatus);
  }
  if (q) {
    const pattern = `%${q}%`;
    conditions.push("(name LIKE ? OR name_kana LIKE ? OR emp_no LIKE ?)");
  ...
  const [emp, idRows] = await Promise.all([
    c.env.DB.prepare("SELECT * FROM employees WHERE id = ?").bind(id).first(),
    (navParams.length ? idStmt.bind(...navParams) : idStmt).all()
  ]);
  if (!emp)
    return c.text("\u793E\u54E1\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093", 404);
  ...
  if (q)
    qsObj.set("q", q);
  if (filterDiv !== "all")
    qsObj.set("div", filterDiv);
  if (filterStatus !== "all")
    qsObj.set("enrollment", filterStatus);
  if (filterActive !== "1")
    qsObj.set("active", filterActive);
  if (filterRetirement)
    qsObj.set("retire", filterRetirement);
  const qsStr = qsObj.toString();
  return c.html(layout(`${emp.name} \u2014 \u793E\u54E1\u60C5\u5831`, staffForm(emp, nav, qsStr), "staff"));
});
function staffForm(emp, nav, qsStr) {
  const isNew = !emp;
  ...
  const enrollOptions = ["\u901A\u5E38", "\u80B2\u4F11", "\u75C5\u6B20", "\u50B7\u75C5", "\u9577\u6B20"].map(
...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;gap:8px;">
  ${nav.prevId ? `<a href="${ADMIN_PATH}/staff/${nav.prevId}${qsStr ? "?" + qsStr : ""}" style="display:flex;align-items:center;gap:5px;padding:7px 14px;background:#f1f5f9;color:#374151;border:1px solid #d1d5db;border-radius:8px;text-decoration:none;font-size:12px;font-weight:600;max-width:44%;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">\u2190 ${escHtml(nav.prevName ?? "\u524D\u306E\u793E\u54E1")}</a>` : `<span></span>`}
  ${nav.nextId ? `<a href="${ADMIN_PATH}/staff/${nav.nextId}${qsStr ? "?" + qsStr : ""}" style="display:flex;align-items:center;gap:5px;padding:7px 14px;background:#f1f5f9;color:#374151;border:1px solid #d1d5db;border-radius:8px;text-decoration:none;font-size:12px;font-weight:600;max-width:44%;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;">${escHtml(nav.nextName ?? "\u6B21\u306E\u793E\u54E1")} \u2192</a>` : `<span></span>`}
</div>` : "";
  return `
<div style="max-width:720px;font-family:'Hiragino Sans','Meiryo',sans-serif;">
  ${navBar}
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;justify-content:flex-end;">
      ${emp.is_active && isNewcomer ? `<button onclick="toggleNewcomer(${emp.id},true,'${escHtml(emp.name)}')" style="padding:5px 14px;background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe;border-radius:6px;font-size:12px;cursor:pointer;">\u65B0\u4EBA\u89E3\u9664\uFF08\u4E00\u822C\u793E\u54E1\u306B\u5909\u66F4\uFF09</button>` : emp.is_active ? `<button onclick="toggleNewcomer(${emp.id},false,'${escHtml(emp.name)}')" style="padding:5px 14px;background:#dbeafe;color:#1d4ed8;border:1px solid #93c5fd;border-radius:6px;font-size:12px;font-weight:600;cursor:pointer;">\u65B0\u4EBA\u3068\u3057\u3066\u767B\u9332</button>` : ""}
...
app5.put("/:id", async (c) => {
  ...
  if (data.exclude_retirement_candidate !== void 0) {
    sets.push("exclude_retirement_candidate = ?");
    vals.push(data.exclude_retirement_candidate);
  }
  if (data.is_hanchyo !== void 0) {
    sets.push("is_hanchyo = ?");
    vals.push(data.is_hanchyo);
  }
  if (sets.length === 0)
    return c.json({ ok: true });
...
app5.post("/csv-import", async (c) => {
  ...
  for (let ci = 0; ci < valid.length; ci += LOOKUP_CHUNK) {
    ...
    ).bind(...lc.map((e) => e.emp_no)).all();
    for (const r of rows.results ?? [])
      existingSet.add(r.emp_no);
  }
...
app5.post("/retire-by-empno", async (c) => {
  const data = await c.req.json();
  if (!Array.isArray(data?.empNos) || data.empNos.length === 0)
    return c.json({ error: "emp_no\u304C\u6307\u5B9A\u3055\u308C\u3066\u3044\u307E\u305B\u3093" }, 400);
  const valid = data.empNos.filter((n) => /^\d{8}$/.test(n));
  if (valid.length === 0)
    return c.json({ error: "\u6709\u52B9\u306Aemp_no\u304C\u3042\u308A\u307E\u305B\u3093" }, 400);
...
app5.post("/purge-by-empno", async (c) => {
  const data = await c.req.json();
  if (!Array.isArray(data?.empNos) || data.empNos.length === 0)
    return c.json({ error: "emp_no\u304C\u6307\u5B9A\u3055\u308C\u3066\u3044\u307E\u305B\u3093" }, 400);
  const valid = data.empNos.filter((n) => /^\d{8}$/.test(n));
...
app5.post("/bulk-retire", async (c) => {
  try {
    ...
    if (ids.length === 0)
      return c.json({ error: "\u6709\u52B9\u306AID\u304C\u3042\u308A\u307E\u305B\u3093" }, 400);
    const CHUNK = 100;
    let total = 0;
    for (let i = 0; i < ids.length; i += CHUNK) {
      const chunk = ids.slice(i, i + CHUNK);
...
__name(subHeader, "subHeader");
app18.get("/settings/liff", async (c) => {
  ...
  const rows = all.map((u) => {
    ...
    const options = Object.entries(ROLE_LABELS).map(
      ([r, l]) => `<option value="${r}" ${r === role ? "selected" : ""}>${escHtml(l)}</option>`
    ).join("");
...
app18.get("/settings/lost-items", async (c) => {
  const typeFilter = c.req.query("type") ?? "";
  const statusFilter = c.req.query("status") ?? "";
  const cookie = c.req.header("Cookie") ?? null;
  const sid = getSessionFromCookie(cookie);
  const adminId = sid ? await validateSession(c.env.DB, sid) : null;
  const adminRow = adminId ? await c.env.DB.prepare("SELECT username FROM admins WHERE id = ?").bind(adminId).first() : null;
  ...
  const rows = all.map((r) => {
    ...
    const empStr = r.employee_name ? `${r.employee_division ? r.employee_division + "\u8AB2" : ""}${r.employee_team ? r.employee_team + "\u73ED" : ""} ${r.employee_name}` : "\u2014";
    const resolvedInfo = r.status === "resolved" && r.resolved_by_name ? `<div style="font-size:11px;color:#059669;margin-top:2px;">\u5BFE\u5FDC: ${escHtml(r.resolved_by_name)}${r.resolved_at ? " " + r.resolved_at.slice(5, 16) : ""}</div>` : "";
    return `<tr>
  ...
  const buildUrl = /* @__PURE__ */ __name((t, s) => `${ADMIN_PATH}/settings/lost-items?type=${t}&status=${s}`, "buildUrl");
  const filterBtn = /* @__PURE__ */ __name((label, t, s) => {
...
app18.get("/settings/accidents", async (c) => {
  const cookie = c.req.header("Cookie") ?? null;
  const sid = getSessionFromCookie(cookie);
  const adminId = sid ? await validateSession(c.env.DB, sid) : null;
  const adminRow = adminId ? await c.env.DB.prepare("SELECT username FROM admins WHERE id = ?").bind(adminId).first() : null;
...
app18.put("/api/liff-users/:id/role", async (c) => {
  const id = parseInt(c.req.param("id"));
  ...
  const user = await c.env.DB.prepare("SELECT line_uid FROM line_liff_users WHERE id = ?").bind(id).first();
  if (user) {
    await reassignRichMenu(user.line_uid, role, c.env);
  }
  return c.json({ ok: true });
});
app18.delete("/api/liff-users/:id", async (c) => {
  const id = parseInt(c.req.param("id"));
  const user = await c.env.DB.prepare("SELECT line_uid FROM line_liff_users WHERE id = ?").bind(id).first();
  if (user) {
...
app18.put("/api/liff/lost-items/:id/status", async (c) => {
  const id = parseInt(c.req.param("id"));
  const { status, resolvedBy } = await c.req.json();
...
app18.put("/api/liff/accident-reports/:id/status", async (c) => {
  const id = parseInt(c.req.param("id"));
  const { status, resolvedBy } = await c.req.json();
  if (status !== "open" && status !== "resolved")
    return c.json({ error: "invalid" }, 400);
...
async function reassignRichMenu(lineUid, role, env) {
  const at = env.LINE_CHANNEL_ACCESS_TOKEN ?? "";
  if (!at)
    return;
...
__name(reassignRichMenu, "reassignRichMenu");
function getRichMenuForRole(role, env) {
  switch (role) {
    case "newcomer":
      return env.RICHMENU_ID_PATTERN1 ?? "";
    case "operations_manager":
      return env.RICHMENU_ID_PATTERN2 ?? "";
    case "general_manager":
      return env.RICHMENU_ID_PATTERN3 ?? "";
    default:
      return "";
  }
}
__name(getRichMenuForRole, "getRichMenuForRole");
var admin_liff_default = app18;
...
function formatVehicleResults(query, vehicles) {
  ...
  for (const v of vehicles) {
    ...
    if (v.division || v.team) {
      const divTeam = v.division ? v.team ? `${v.division}${v.team}\u73ED` : v.division : `${v.team}\u73ED`;
      lines.push(divTeam);
    }
...
async function registerLiffUser(db, lineUid, name, role, empId, env) {
  await db.prepare(`
...
__name(registerLiffUser, "registerLiffUser");
...
async function handleRegistrationFlow(env, lineUid, replyToken, at, inputText, state, data) {
...
__name(handleRegistrationFlow, "handleRegistrationFlow");
async function handleUnregisteredUser(env, lineUid, replyToken, at, inputText, state, data) {
...
__name(handleUnregisteredUser, "handleUnregisteredUser");
async function handleOperationsUser(env, lineUid, replyToken, at, inputText, state, data, liffUser) {
  if (inputText === "\u30AD\u30E3\u30F3\u30BB\u30EB" || inputText === "cancel") {
    await setState(env.DB, lineUid, "idle");
  ...
  if (inputText === "\u5FD8\u308C\u7269\u5BFE\u5FDC" || inputText === "\u5FD8\u308C\u7269") {
    ...
    if (url) {
      await reply(replyToken, at, [text(`\u{1F4E6} \u5FD8\u308C\u7269\u5BFE\u5FDC\u30D5\u30A9\u30FC\u30E0
...
${url}`)]);
  ...
  if (inputText === "\u4E8B\u6545\u5831\u544A" || inputText === "\u4E8B\u6545") {
    ...
    const url = liffId ? `https://liff.line.me/${liffId}` : "";
    if (url) {
      await reply(replyToken, at, [text(`\u{1F6A8} \u4E8B\u6545\u5831\u544A\u30D5\u30A9\u30FC\u30E0
...
__name(handleOperationsUser, "handleOperationsUser");
async function handleVehicleManager(env, lineUid, replyToken, at, inputText) {
  if (/^\d{1,6}$/.test(inputText)) {
    const vehicles = await searchVehicles(env.DB, inputText);
    await reply(replyToken, at, [text(formatVehicleResults(inputText, vehicles))]);
    return;
...
__name(handleVehicleManager, "handleVehicleManager");
async function handleNewcomer(env, lineUid, replyToken, at, inputText, state, data, liffUser) {
...
`;
  ...
  if (curState === "sales_confirm_overwrite") {
    if (inputText === "\u4E0A\u66F8\u304D") {
      await setState(env.DB, lineUid, "sales_amount", { date: curData.date });
...
\u{1F4C5} \u65E5\u4ED8: ${curData.date}
\u{1F4B0} \u58F2\u4E0A: ${curData.amount.toLocaleString("ja-JP")}\u5186
\u{1F695} \u4E57\u8ECA: ${curData.ride_count}\u56DE
...
\u767B\u9332\u3057\u307E\u3059\u304B\uFF1F`,
  ...
  if (curState === "sales_confirm") {
    if (inputText === "\u767B\u9332") {
      ...
      const { year, month } = getPeriod(curData.date);
      await env.DB.prepare(`
        INSERT INTO sales_records (emp_id, date, amount, ride_count, distance_km, period_year, period_month, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now', 'localtime'))
        ON CONFLICT(emp_id, date) DO UPDATE SET
...
${curData.date}
\u58F2\u4E0A: ${curData.amount.toLocaleString("ja-JP")}\u5186`)]);
    } else {
      await setState(env.DB, lineUid, "idle");
      await reply(replyToken, at, [text("\u30AD\u30E3\u30F3\u30BB\u30EB\u3057\u307E\u3057\u305F\u3002")]);
    }
...
__name(handleNewcomer, "handleNewcomer");
async function handleUnknownRole(replyToken, at, name) {
  await reply(replyToken, at, [text(
...
__name(handleUnknownRole, "handleUnknownRole");
...
app19.get("/api/liff/staff-lookup", async (c) => {
  const uid = await uidFromRequest(c.req.raw);
  if (!uid)
    return c.json({ error: "unauthorized" }, 401);
  const liffUser = await c.env.DB.prepare(
...
app19.post("/api/liff/staff-edit", async (c) => {
  const uid = await uidFromRequest(c.req.raw);
  if (!uid)
    return c.json({ error: "unauthorized" }, 401);
  const liffUser = await c.env.DB.prepare(
  ...
  if (!body.id || !body.name)
    return c.json({ error: "id \u3068 name \u306F\u5FC5\u9808\u3067\u3059" }, 400);
  await c.env.DB.prepare(`
    UPDATE employees SET
...
app19.post("/api/liff/staff-retire", async (c) => {
  const uid = await uidFromRequest(c.req.raw);
  if (!uid)
    return c.json({ error: "unauthorized" }, 401);
  const liffUser = await c.env.DB.prepare(
...
app19.post("/api/liff/staff-add", async (c) => {
  const uid = await uidFromRequest(c.req.raw);
  if (!uid)
    return c.json({ error: "unauthorized" }, 401);
  const liffUser = await c.env.DB.prepare(
  ...
  if (existing)
    return c.json({ error: "\u3053\u306E\u793E\u54E1\u756A\u53F7\u306F\u3059\u3067\u306B\u767B\u9332\u3055\u308C\u3066\u3044\u307E\u3059" }, 409);
  await c.env.DB.prepare(`
...
app19.post("/api/liff/lost-item", async (c) => {
  const uid = await uidFromRequest(c.req.raw);
  if (!uid)
    return c.json({ error: "unauthorized" }, 401);
  const liffUser = await c.env.DB.prepare(
  ...
  if (at)
    await pushMessage(uid, at, summary);
  return c.json({ ok: true, summary });
});
app19.post("/api/liff/accident", async (c) => {
  const uid = await uidFromRequest(c.req.raw);
  if (!uid)
    return c.json({ error: "unauthorized" }, 401);
  const liffUser = await c.env.DB.prepare(
...
function buildLostItemSummary(body) {
  const lines = [];
  ...
  if (body.received_at)
    lines.push(`\u53D7\u96FB: ${body.received_at}`);
  if (body.vehicle_no)
    lines.push(`\u8ECA\u756A: ${body.vehicle_no}`);
  ...
  if (body.item_description)
    lines.push(`\u5FD8\u308C\u7269: ${body.item_description}`);
  if (body.pickup_location)
    lines.push(`\u4E57\u8ECA\u5730: ${body.pickup_location}`);
  if (body.dropoff_location)
    lines.push(`\u964D\u8ECA\u5730: ${body.dropoff_location}`);
  if (body.customer_name)
    lines.push(`\u5BA2\u540D: ${body.customer_name}`);
  if (body.customer_phone)
    lines.push(`\u96FB\u8A71: ${body.customer_phone}`);
  if (body.return_method)
    lines.push(`\u8FD4\u5374\u65B9\u6CD5: ${body.return_method}`);
  if (body.notes)
    lines.push(`\u5099\u8003: ${body.notes}`);
  return lines.join("\n");
}
__name(buildLostItemSummary, "buildLostItemSummary");
function buildAccidentSummary(body) {
  const lines = ["\u3010\u4E8B\u6545\u5831\u544A\u3011"];
  if (body.received_at)
    lines.push(`\u53D7\u96FB: ${body.received_at}`);
  if (body.vehicle_no)
  ...
  if (body.accident_type)
    lines.push(`\u4E8B\u6545\u5F62\u614B: ${body.accident_type}`);
  if (body.car_status)
    lines.push(`\u72B6\u614B: ${body.car_status}`);
  if (body.location)
    lines.push(`\u5834\u6240: ${body.location}`);
...
__name(buildAccidentSummary, "buildAccidentSummary");
function liffLostItemPage(liffId) {
  return `<!DOCTYPE html>
<html lang="ja">
<head>
  <meta charset="UTF-8">
...
__name(liffLostItemPage, "liffLostItemPage");
function liffAccidentPage(liffId) {
  return `<!DOCTYPE html>
<html lang="ja">
<head>
  <meta charset="UTF-8">
...
__name(liffAccidentPage, "liffAccidentPage");
function liffStaffLookupPage(liffId) {
  return `<!DOCTYPE html>
<html lang="ja">
<head>
  <meta charset="UTF-8">
  ...
  <div class="view" id="view-search" style="display:none;">
    ...
    <div class="search-area">
      <div class="search-box">
        <span style="color:#9ca3af;font-size:16px;flex-shrink:0;">\u{1F50D}</span>
...
__name(liffStaffLookupPage, "liffStaffLookupPage");
var liff_default = app19;

// src/routes/admin_inspection.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
var app20 = new Hono2();
function inspectionPage(adminPath) {
  return `
<style>
...
<div id="ins-panel-input" class="ins-panel active">
  ...
  <div class="ins-data-tools">
    ...
    <button class="btn-t red" onclick="insClearMonth()">\u{1F5D1} \u5168\u524A\u9664</button>
  </div>
</div>
...
<div id="ins-panel-output" class="ins-panel">
  <div class="ins-controls">
    ...
    <button class="btn-xl" onclick="insCopyImage()">\u{1F4CB} \u30AF\u30EA\u30C3\u30D7\u30DC\u30FC\u30C9\u306B\u30B3\u30D4\u30FC</button>
    <button class="btn-p" onclick="insDownloadImage()">\u{1F4BE} \u753B\u50CF\u4FDD\u5B58</button>
...
async function insRenderCanvas(){
  ...
  function fillCell(secY,secH,vx,vw,tx,tw,vehicles){
    ...
    const lh=43; // 1\u884C\u306E\u9AD8\u3055\uFF0816pt\xD72.489\u225240px + \u4F59\u767D\uFF09
...
__name(inspectionPage, "inspectionPage");
app20.get("/inspection", (c) => {
  return c.html(layout("\u70B9\u691C\u7BA1\u7406", inspectionPage(ADMIN_PATH), "inspection"));
});
var admin_inspection_default = app20;

// src/routes/api/inspection.ts
init_strip_cf_connecting_ip_header();
init_modules_watch_stub();
var app21 = new Hono2();
app21.get("/schedule", async (c) => {
...
app22.get("/robots.txt", (c) => c.text("User-agent: *\nDisallow: /\n"));
app22.use(`/${SECRET}/admin/*`, async (c, next) => {
...
app22.route(`/${SECRET}/admin`, admin_default);
app22.route(`/${SECRET}/admin`, admin_extra_default);
app22.route(`/${SECRET}/admin`, admin_staff_default);
app22.route(`/${SECRET}/admin`, admin_liff_default);
app22.route(`/${SECRET}/admin`, admin_inspection_default);
...
app22.route("/api/shift", shift_default);
app22.route("/api/instructor-schedule", instructor_default);
app22.route("/api/employees", employees_default);
app22.route("/api/sales", sales_default);
app22.route("/api/info", info_default);
app22.route("/api/events", events_default);
app22.route("/api/line", line_api_default);
app22.route("/api/schedule-types", schedule_types_default);
app22.route("/api/interviews", interviews_default);
app22.route("/api/coaches", coaches_default);
app22.route("/api/instructors", instructors_default);
app22.route("/api/period-settings", period_settings_default);
app22.route("/api/notifications", notifications_default);
app22.route("/api/instructor-invite", instructor_invite_default);
app22.route("/api/inspection", inspection_default);
app22.post("/api/line/webhook", async (c) => {
...
app22.route("", liff_default);
app22.get("/", (c) => c.redirect(`${ADMIN_PATH}/login`));
var src_default = {
  fetch: app22.fetch.bind(app22),
