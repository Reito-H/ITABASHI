// scripts/summer_safety/apply_remote.mjs
// migration_134.sql を本番 D1 (staff-db) に適用してから wrangler deploy する。
//   node scripts/summer_safety/apply_remote.mjs
...
const sqlPath = join(systemDir, 'src', 'db', 'migration_134.sql');
...
console.log(`migration_134: ${statements.length} 文を本番 staff-db へ適用します`);
...
execFileSync(
  ...
  ['wrangler', 'd1', 'execute', 'staff-db', '--remote', '--yes', '--command',
    'SELECT (SELECT COUNT(*) FROM summer_safety_2026_people) AS people, (SELECT COUNT(*) FROM summer_safety_2026_entries) AS entries, (SELECT COUNT(*) FROM summer_safety_2026_entries WHERE value=3) AS n3;'],
